...
\begin{tikzpicture}[node distance = 3cm, auto]
    ...
    \node [block, below of=groupdata] (estatgeral) {obtem estatísticas gerais};
    ...
    \path [line] (decideic) -| node {não} (processrod);
    \path [line] (decideic) -- node {sim} (groupdata);
    \path [line] (groupdata) -- (estatgeral);
...
\begin{tikzpicture}[node distance = 3cm, auto]
    ...
    \node [cloud, below of=decide, node distance=3cm] (stop) {fim};
    % Draw edges
